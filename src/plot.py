...
def _to_percent(v: Optional[float]) -> Optional[float]:
    ...
    try:
        ...
        return None
    # If value looks like fraction (<= 1.0), scale to percent
...
def draw_dashboard(root: Dict[str, Any], years: List[int], by_year: Dict[int, Dict[str, Any]]):
    # Assemble annual series
    occ = []
    adr = []
    rvp = []
    for y in years:
        ann = get_annual_summary(by_year[y])
        occ.append(ann["occupancy_pct"])  # percent
        adr.append(ann["adr_jpy"])        # JPY
        rvp.append(ann["revpar_jpy"])     # JPY
    ...
    ax_occ.plot(years, occ, marker="o", color="#1f77b4")
    if years:
    ...
    ax_adr.plot(years, adr, marker="o", color="#2ca02c")
    if years:
    ...
    ax_rvp.plot(years, rvp, marker="o", color="#ff7f0e")
    if years:
    ...
    # Special notes markers (ticks only)
    for y in years:
        notes = by_year[y].get("special_notes") or []
        if notes:
            ax_occ.plot([y, y], [0, 0.5], color="#444", alpha=0.4)
            ax_adr.plot([y, y], [0, 0.5], color="#444", alpha=0.2)
            ax_rvp.plot([y, y], [0, 0.5], color="#444", alpha=0.2)
...
def draw_trends_combined(years: List[int], by_year: Dict[int, Dict[str, Any]]):
    ...
    ax.plot(years, occ, marker="o", label="Occ (%)", color="#1f77b4")
    ax2 = ax.twinx()
    ax2.plot(years, adr, marker="o", label="ADR (JPY)", color="#2ca02c")
    ax2.plot(years, rvp, marker="o", label="RevPAR (JPY)", color="#ff7f0e")
    if years:
...
def draw_per_year_monthlies(years: List[int], by_year: Dict[int, Dict[str, Any]]):
    # One figure per year with 3 subplots
    for y in years:
        ms = get_monthly_series(by_year[y])
        ...
        # Occupancy vs RevPAR/ADR overlay
        ax1.set_title("Occupancy (%)")
        ax1.plot(months, ms["occ_pct"], marker="o", color="#1f77b4")
        ax1.set_xlim(1, 12)
        ...
        ax2.plot(months, ms["adr_jpy"], marker="o", color="#2ca02c", label="ADR")
        ax2.plot(months, ms["revpar_jpy"], marker="o", color="#ff7f0e", label="RevPAR")
        ax2.set_xlim(1, 12)
...
def draw_data_completeness(years: List[int], by_year: Dict[int, Dict[str, Any]]):
    ...
    fig, ax = plt.subplots(figsize=(10, 4.2))
    ax.bar(years, comp_counts, color="#6baed6")
    ax.set_ylim(0, 12)
